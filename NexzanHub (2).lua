...
function NexzanHub:ApplyTheme()
    ...
    for obj, map in pairs(ThemeRegistry) do
        if obj and obj.Parent ~= nil or (typeof(obj) == "Instance" and pcall(function() return obj.Name end)) then
...
function NexzanHub:CreateWindow(cfg)
    ...
    if type(ToggleKey) == "string" then
        ToggleKey = Enum.KeyCode[ToggleKey] or Enum.KeyCode.RightControl
    end
    ...
    function Window:AddTab(tabCfg)
        ...
        function Tab:AddSection(secCfg)
            ...
            function Section:AddButton(bCfg)
                ...
                Register(row, { BackgroundColor3 = "Element" }, true)
            ...
            function Section:AddKeybind(kCfg)
                ...
                if type(default) == "string" then
                    default = Enum.KeyCode[default] or Enum.UserInputType[default]
                end
                    ...
                    if listening then
                        ...
                        elseif candidate == Enum.KeyCode.Backspace or candidate == Enum.KeyCode.Delete then
                            ...
                            for _, b in ipairs(blacklist) do
                                local bEnum = type(b) == "string" and (Enum.KeyCode[b] or Enum.UserInputType[b]) or b
                                if bEnum == candidate then banned = true break end
                            end
                            if not banned then
                ...
                function api:Set(newKey)
                    if type(newKey) == "string" then newKey = Enum.KeyCode[newKey] or Enum.UserInputType[newKey] end
                    SetKey(newKey, true)
                end
    ...
    if keyCfg and (keyCfg.Enabled ~= false) then
        ...
        if savedOk then
            ...
            Register(verifyBtn, { BackgroundColor3 = "Accent" })
            Register(verifyBtn, { TextColor3 = "Title" })
            ...
            if getKeyLink then
                ...
                Register(getKeyBtn, { BackgroundColor3 = "Element" })
                Register(getKeyBtn, { TextColor3 = "Text" })
